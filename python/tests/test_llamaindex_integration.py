...
class TestMemoClawRetriever:
    def test_basic_retrieval(self, mock_client: MagicMock):
        ...
        assert nodes[0].node.text == "User prefers dark mode"
    ...
    def test_retrieval_returns_text_nodes(self, mock_client: MagicMock):
        mock_client.recall.return_value = RecallResponse(
            memories=[
                _make_recall_memory(id="mem-1", content="test content"),
            ],
            query_tokens=5,
        )
        retriever = MemoClawRetriever(client=mock_client)
        ...
        assert nodes[0].node.id_ == "mem-1"
        assert nodes[0].node.text == "test content"
...
class TestMemoClawMemoryStore:
    ...
    def test_put_batch(self, mock_client: MagicMock):
        ...
        store.put_batch([
            {"content": "Memory 1"},
            {"content": "Memory 2", "namespace": "other-ns"},
        ])

        mock_client.store_batch.assert_called_once()
        items = mock_client.store_batch.call_args[0][0]
        assert len(items) == 2
        assert items[0]["namespace"] == "ns"  # inherited
        assert items[1]["namespace"] == "other-ns"  # explicit

    def test_put_batch_inherits_session_and_agent(self, mock_client: MagicMock):
        mock_client.store_batch.return_value = MagicMock()
        store = MemoClawMemoryStore(
            client=mock_client,
            namespace="ns",
    ...
    def test_search_basic(self, mock_client: MagicMock):
        mock_client.recall.return_value = RecallResponse(
            memories=[
                _make_recall_memory(
                    id="mem-1",
    ...
    def test_search_inherits_defaults(self, mock_client: MagicMock):
        mock_client.recall.return_value = RecallResponse(
            memories=[], query_tokens=5
        )
        store = MemoClawMemoryStore(
    ...
    def test_delete_batch(self, mock_client: MagicMock):
        mock_client.delete_batch.return_value = MagicMock()
        store = MemoClawMemoryStore(client=mock_client)
    ...
    def test_as_retriever(self, mock_client: MagicMock):
        ...
        retriever = store.as_retriever(top_k=10, min_similarity=0.8)
        ...
        assert retriever._min_similarity == 0.8

    def test_as_retriever_with_tags(self, mock_client: MagicMock):
        store = MemoClawMemoryStore(client=mock_client)
        retriever = store.as_retriever(tags=["important"], include_relations=True)
